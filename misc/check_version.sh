current_version=$(grep "__version__" ../structuremap/__init__.py | cut -f3 -d ' ' | sed 's/"//g')
...
set +e
already_on_pypi=$(pip install structuremap== 2>&1 | grep -c "$current_version_as_regex")
set -e
conda deactivate
